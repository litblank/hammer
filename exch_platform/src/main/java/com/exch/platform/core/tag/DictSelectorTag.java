...
package com.exch.platform.core.tag;
...
public class DictSelectorTag extends Tag {
    ...
    public void render() {
        ...
        //字典类型列表
        String dict = ToolUtil.isNotEmpty(attrs.get("isdict")) ? attrs.get("isdict").toString() : "";
        ...
        if(StringUtils.isNotEmpty(dict)){
            ...
            wrapper.eq("pid","0");
            list = iDictService.selectList(wrapper);
        }else{
            list = iDictService.selectByParentCode(code);
        }
