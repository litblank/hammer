 ...
 * 表格点击事件
 */
Role.monclick = function(row, $element,field){
        $("#role_assign").attr("src","role/role_assign/"+ row.id);
        $("#role_assign").attr("data-id","role/role_assign/"+ row.id);
};
/**
 * 权限配置
 */
...
Role.assignUser = function(){
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/mgr/assignUserTo/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
$(function () {
    ...
    var table = new BSTable(Role.id, "/role/list", defaultColunms,Role.monclick);
